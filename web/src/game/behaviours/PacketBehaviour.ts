...
/**
 * Minimum surface `PacketBehaviour` needs from a mask. Both the game's
...
/**
 * The session-facing interface `PacketBehaviour` needs. Only two hooks are
 ...
 */
export class PacketBehaviour extends Behaviour {
  private readonly session: PacketSessionHooks
  private mode: PacketMode = 'growing'
  ...
  private motionTrail: PacketMotionTrailNode | null = null
  private hexParticles: ParticleEmitterNode | null = null
  /**
   ...
   * unchanged.
   */
  private readonly autonomousDrift: boolean
  /**
   ...
   * GC at 120 Hz.
   */
  private readonly _hexSpeedRange: [number, number] = [0, 0]
   ...
   * behaviour feeds the trail world-space samples in `onFixedStep` and the
   ...
   * behaviour drives `emitter.setOrigin` and `emitter.config.emitDirectionRad`
   ...
   * the head as it advances. Called by `PathDrawBehaviour` when the player
   * finishes a drag.
   */
  ...
  /** Currently-bound trail, read by `PathDrawBehaviour` on resume. */
  ...
  private readonly _trailScratch: { x: number; y: number } = { x: 0, y: 0 }

  private applyBorderTurnaround(x: number, y: number, fdt: number): void {
  ...
  private startCapture(packet: PacketNode, center: Vec2): void {
    this.mode = 'captured'
