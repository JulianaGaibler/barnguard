...
export interface DriftPacketOpts {
  host: EngineHost
  /** Where the packet + its trail mount. */
  layer: Node2D
  pos: Vec2
  headingRad: number
  speed: number
  /** Unique id for the debug Scene panel. */
  id: string
  onDestroy?: (packet: PacketNode) => void
}
...
export function spawnDriftPacket(opts: DriftPacketOpts): PacketNode {
  return spawnPacketInSession(
    {
      host: opts.host,
      packetLayer: opts.layer,
      hooks: {
        isPlaying: () => true,
        epicenter: () => null,
        gameViewport: () => DRIFT_BOUNDS,
        mask: () => DRIFT_MASK,
        onExitedGermany: () => {},
        onCaptured: () => {},
      },
