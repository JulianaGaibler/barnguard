...
export interface PathDrawSessionHooks {
  ...
  /**
   * Convert an arcade-world pointer coord into the map-local space the packets,
   * trails, and epicenter all live in (the game sits under a scaled parent in
   * the arcade). Trail points and snap checks operate in map space.
   */
...
export class PathDrawBehavior extends PointerBehavior {
  ...
  #handleMove(e: PointerEvent2D): void {
    ...
    const world = this.#session.worldToMap(e.pointer.world.x, e.pointer.world.y)
    if (ep) {
  ...
  #spawnHandle(x: number, y: number): void {
    ...
    handle.addBehavior(
      new EndpointResumeBehavior(this.node as PacketNode, {
        ...
        worldToMap: (wx, wy) => this.#session.worldToMap(wx, wy),
      }),
    )
