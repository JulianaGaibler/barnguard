...
const badgesOf = (g: Group, cards: readonly Card[] = DECK) =>
...
describe('group distribution', () => {
  ...
  it('matches the per-group badge counts', () => {
    expect(ALL_GROUPS.map((g) => badgesOf(g))).toEqual([15, 17, 17, 18, 18, 20])
    expect(ALL_GROUPS.reduce((n, g) => n + badgesOf(g), 0)).toBe(105)
  })
  ...
  it('matches the badge split by floor', () => {
    const split = ALL_GROUPS.map(
      (g) =>
        `${badgesOf(g, onFloor('management'))}+${badgesOf(g, onFloor('ic'))}`,
    )
