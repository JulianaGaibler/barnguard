...
describe('per-metric scoring', () => {
  it('counts the scoring card own group', () => {
    // Chief Operating Officer: 4 per Leadership badge in its row, itself included.
  ...
  it('counts a set as the smaller of its two halves', () => {
    ...
    // and two Product badges make exactly one pair.
    const grid = emptyGrid()
    grid[1]![1] = seat('mgmt-product-operations-manager') // product
    grid[0]![0] = seat('mgmt-board-member') // leadership x2 -> 1 card, 2 badges
...
describe('regions', () => {
  it('counts a rowOrColumn card once, not twice', () => {
    // UX Lead scores per Design badge in its row, so a five-cell rowOrColumn
    // scope must not double-count the scoring seat itself.
    const grid = emptyGrid()
...
describe('runs of identical groups', () => {
  it('pays per three matching badges, summed over groups', () => {
    // Head of Product: 6 per run of three identical group badges.
    const grid = emptyGrid()
