...
describe('resolveAbility', () => {
  ...
  it('counts the triggering card itself', () => {
    // Chief Operating Officer: 1 budget per Leadership badge, itself included.
    const g = grid3()
...
// Funding pays the player rather than the seats, since a budget line holds
// nothing until the settlement. What the seats still decide is the ceiling.
describe('funding budget lines', () => {
  it('pays for the fullest-capacity lines when the target is a count', () => {
    ...
    expect(out.selfBudget).toBe(13) // 9 + 4, leaving the cap-3 line alone
  })
