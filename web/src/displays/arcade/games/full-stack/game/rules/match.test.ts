...
describe('undo', () => {
  ...
  it('is exact across a reshuffle', () => {
    ...
    const turn = legalTurns(state).find(
      (t) => t.approval.join() === 'refreshFloor',
    )
...
describe('open seats', () => {
  it('pay budget and approvals instead of costing anything', () => {
    ...
    const turn = legalTurns(state).find(
      (t) => t.take === 'openSeat' && t.approval.length === 0,
    )!
    applyTurn(state, turn)
  ...
  it('can take a candidate the player could not afford', () => {
    ...
    const overpriced = turns.filter(
      (t) =>
        t.take === 'hire' &&
        !t.approval.includes('refreshFloor') &&
...
describe('approval actions', () => {
  it('moving the marker costs one approval and flips the floor', () => {
    ...
    const marker = state.marker
    const turn = legalTurns(state).find(
      (t) => t.approval.join() === 'moveMarker' && t.take === 'openSeat',
    )!
    applyTurn(state, turn)
  ...
  it('is unavailable with no approvals left', () => {
    ...
    expect(legalTurns(state).every((t) => t.approval.length === 0)).toBe(true)
  })

  it('offers both steps on one turn, in either order', () => {
    const state = createMatch(43)
    state.sides[state.turn]!.approvals = 2
    const plans = new Set(legalTurns(state).map((t) => t.approval.join()))
    expect(plans.has('moveMarker,refreshFloor')).toBe(true)
    expect(plans.has('refreshFloor,moveMarker')).toBe(true)
  })

  it('spends one approval per step and redeals the floor it ends on', () => {
    const state = createMatch(47)
    const side = state.sides[state.turn]
    side.approvals = 3
    const marker = state.marker
    const other = marker === 'ic' ? 'management' : 'ic'
    const before = state.shortlists[other].map((c) => c!.id)
    const turn = legalTurns(state).find(
      (t) =>
        t.approval.join() === 'moveMarker,refreshFloor' &&
        t.take === 'openSeat',
    )!
    applyTurn(state, turn)
    // Two spent, then two paid back by the open seat.
    expect(side.approvals).toBe(3 - 2 + 2)
    expect(state.marker).toBe(other)
    const discarded = new Set(state.discards[other].map((c) => c.id))
  ...
  it('refreshing sends the whole showing row to the discard pile', () => {
    ...
    const turn = legalTurns(state).find(
      (t) => t.approval.join() === 'refreshFloor' && t.take === 'openSeat',
    )!
    applyTurn(state, turn)
