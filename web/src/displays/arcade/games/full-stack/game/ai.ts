...
// Full Stack is almost perfect information (no hands, both orgs and both
...
// Re-exported so the existing callers keep importing the generator from here.
export { seededRandom, type Random }

/**
 * The AI's decision for a turn.
 *
 * A plan that redeals carries no hire: the cards it deals are not known yet, so
 * the caller applies the approvals, then asks again with `planHire`.
 */
...
const redeals = (plan: ApprovalPlan): boolean => plan.includes('refreshFloor')

/** Approvals kept back from the two-step plan. */
const SPARE_APPROVAL = 1
 ...
 */
/** What one approval plan is worth, and the hire it leads to. */
export interface ActionScore {
  action: ApprovalPlan
  score: number
  /** Null for a redraw: the cards it deals are not known yet. */
  hire: Hire | null
  /** Determinizations averaged. 1 for the plans with nothing hidden. */
  samples: number
}

/**
 * Value every approval plan open to the player to move.
 *
 * The known-information plans are worth the best hire that follows them. A plan
 * that redeals is worth the average, over sampled deals, of the best hire that
 * follows, which is what makes it comparable: you commit the approval first and
 * only then see what you get.
 *
 * Switching and then redealing is searched, since it deals a new row on the
 * floor the hire comes from. The reverse order is not: it pays two approvals to
 * refresh a row this turn leaves behind, and no evaluation here can see the
 * denial that would have to justify it.
 */
export function* scoreActions(
  ...
  const approvals = state.sides[me].approvals
  const plans: ApprovalPlan[] = [[]]
  if (approvals >= 1) {
    plans.push(['moveMarker'])
    if (cfg.considerRedraw) plans.push(['refreshFloor'])
  }
  // Switching and then redealing is two approvals for a random row, where a
  // plain redeal is one, so it only pays when this floor is the wrong floor and
  // the other one is showing nothing either. Held back until there is an
  // approval to spare, because the value of a redeal is an average over sampled
  // deals and the value of the alternatives is exact: offered on equal terms,
  // the noisier estimate wins the tie too often and the search spends itself
  // down to nothing.
  if (approvals >= SPARE_APPROVAL + 2 && cfg.considerRedraw) {
    plans.push(['moveMarker', 'refreshFloor'])
  }
...
export function* planTurn(
  ...
  const best = scored[0]!
  if (redeals(best.action)) return { approval: best.action, hire: null }
  if (!best.hire) return null
  return { approval: best.action, hire: best.hire }
}
