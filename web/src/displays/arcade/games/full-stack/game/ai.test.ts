...
describe('chooseTurn', () => {
  // The timeout is raised because a hard match runs a real search on every
  // turn, which passes the default budget on a loaded machine.
  it.each(DIFFICULTIES)(
    'plays a legal full match on %s',
    (difficulty) => {
  ...
  it('never rerolls on easy', () => {
    ...
    while (!isOver(state)) {
      expect(takeAiTurn(state, 'easy', rng)!.approval).not.toContain(
        'refreshFloor',
      )
    }
  })
