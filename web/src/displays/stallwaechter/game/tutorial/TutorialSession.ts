 ...
 * `PacketBehavior::applyBorderTurnaround` never engages in the tutorial. The
...
/**
 * Mini-game inside the state-confirm card. Secondary `Stage`, one packet left,
 * one epicenter right, reuses the main-game packet/path stack via
 * `spawnPacketInSession` so visual and dynamics changes flow through.
 *
 * Packet uses `autonomousDrift: false`, sits at v=0 until drawn. On capture or
 * viewport exit it retires and respawns after `RESPAWN_DELAY_SEC`.
...
export class TutorialSession {
  ...
  readonly #hint: TutorialHintNode
  readonly #sessionAbort = new AbortController()
  #offStagePointerDown: (() => void) | null = null
  #currentPacket: PacketNode | null = null
  #packetIdSeq = 0
  #destroyed = false
  /**
   ...
   * coherent.
   */
  #handleResize(info: StageResizeInfo): void {
    ...
    const packet = spawnPacketInSession(
      {
        ...
        hooks: {
          ...
          onCaptured: (p) => {
            // Match main-game behavior: session destroys the packet after
        ...
        drawHooks: {
          ...
          bindTrailToPacket: (target, trail) => {
            target.getBehavior(PacketBehavior)?.setTrail(trail)
          },
        },
  ...
  #onPacketRetired(): void {
    // Exit path: PacketBehavior switched the packet to `'lost'` mode and
