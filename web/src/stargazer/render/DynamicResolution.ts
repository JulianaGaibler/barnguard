 ...
 *
 * - Transition: while the camera animates, drop to `motionScale`. On settle, hold
 *   for `settleDwellMs` (so tap-spam doesn't thrash resizes), then ramp up over
 *   `settleStepFrames` (staggered step-up hides the "pop").
 * - Adaptive governor: EMA of raw frame time nudges a baseline scale via
 *   hysteresis, evaluated every `evalIntervalFrames`. Gated off during motion
 *   so zoom cost doesn't drive the steady baseline down.
 *
...
export class DynamicResolution {
  readonly #opts: DynamicResolutionOptions
  ...
  #stepSettle(): number {
    ...
    const value =
      this.#settleFromScale +
      (this.#adaptiveBaseline - this.#settleFromScale) * t
