/**
 * Static-layer offscreen buffer. Holds a bitmap of the current `renderLayer:
 * 'static'` subtree at the current camera + DPR. Each frame the main canvas
 * either blits it (cache hit) or re-bakes (camera settled) / renders live
 * (camera animating).
 *
 * Bakes into an `OffscreenCanvas` and seals into an immutable, GPU-resident
 * `ImageBitmap` via `transferToImageBitmap()`. A live source canvas can force a
 * full GPU readback on every blit in some browsers; sealing to a bitmap avoids
 * that. Steady state holds one bitmap, and the previous is `close()`d on every
 * rebake so VRAM stays bounded.
 *
...
export class Layers {
  ...
  #fallbackCtx: CanvasRenderingContext2D | null = null
  ...
  #sizeChanged(pixelW: number, pixelH: number): boolean {
