...
- **`radius`** is the view-space reach of the scan. Small radii catch tight contact seams, while large
...
- **`directStrength`** (`0`..`1`, default `0`) decides whether AO also dims _direct_ light. At `0`,
  ...
  light stays bright. This is correct, but the occlusion can be hard to see on well-lit ground. Raising it
  folds AO into the diffuse direct lobe as well (never specular), which makes contact darkening read
  everywhere. Treat it as an artistic dial, not a correctness setting.
...
The controller is `render/gfx/ao/AmbientOcclusion.ts`, and the stage runs it
before the frame opens, in the same window as the shadow prepass. It takes the
compute path when `device.supportsCompute` is true and a fragment path
otherwise, and both share the same horizon-scan and depth-aware blur math. The
mesh shaders sample the blurred result and modulate lighting per fragment inside
the MSAA pass, rather than multiplying the composited frame, which would darken
the 2D overlay too.
