...
/**
 * Owns the engine's active tween/wait set, ticked once per render frame in
 * engine time, so tweens freeze when the engine pauses and never drift from the
 * render clock. Every async animation primitive funnels through here, so
 * {@link Animator.cancelAll} on engine destroy rejects every outstanding Promise
 * at once.
 *
...
export class Animator {
  ...
  readonly #_tickScratch: AnimationRecord[] = []
  #_tickDepth = 0
  #_reentrancyWarnings = 0

  /**
   * Tween the numeric properties named in `to` on `target` from their current
   * values to the given values. From-values are snapshotted after any `delay`
   * so the start point isn't disturbed by other writes during the wait.
   * Non-number properties are ignored at runtime (constrain them via TS).
   *
   * @example
   *   const fx = { intensity: 0 }
   *   await animator.tween(
   *     fx,
   *     { intensity: 1 },
   *     { duration: 0.3, easing: easings.outQuad },
   *   )
   */
  ...
  #_runTick(dt: number, snapshot: AnimationRecord[]): void {
  ...
  #finalize(record: AnimationRecord): void {
