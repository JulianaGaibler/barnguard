 ...
 * damped kinematics every frame automatically (the same
 * damp → accel → integrate math as `ParticleEmitter.update`, kept as a
 * separate inline copy here rather than a shared helper — both are tight
 * hot loops over typed arrays with enough surrounding differences that a
 * shared function would just add an indirection). Subclasses add their own
 * parallel typed arrays (spin, shape kind, whatever a specific burst needs)
 * sized to `capacity` and index them with the SAME index the base class
 * hands to `spawnParticle` / `drawParticle` / `updateExtra` / `shouldDespawn`.
 *
 ...
 *       else gfx.strokeLine(-4, 0, 4, 0, { color: '#fff', width: 1, cap: 'round' })
 *     }
 *   }
 */
export abstract class VectorParticleNode extends Node2D {
   ...
   * SYNCHRONOUSLY before moving to the next slot — a subclass may rely on
   * this to stage per-particle state (e.g. a manually-computed spawn angle)
   * in a scratch field immediately before a `burst(1)` call and read it back
   * inside `spawnParticle`. This ordering is a documented contract, not an
   * implementation detail — don't change it to buffer or defer spawns
   * without updating this doc comment.
   */
  ...
  /** Kill a slot now; idempotent (delegates to `SlotPool`'s self-guarding `kill`). Also called automatically when `shouldDespawn` returns true. */
   ...
   * `ParticleEmitter.waitUntilEmpty`: call this RIGHT AFTER the `burst()`
   * you want to wait for, in the same synchronous span, and don't pair it
   * with reuse across unrelated later bursts. A subclass whose
   * `shouldDespawn` never returns `true` will never see this resolve.
   */
  ...
  /**
   * Draw particle `i` in LOCAL SPACE: the base class has already `save()`'d
   * and positioned/rotated the origin at `(x[i], y[i], angle[i])`, so draw as
  ...
  /**
   * Opt-in despawn predicate evaluated once per live particle per frame,
   * after `updateExtra`. Default ALWAYS false — a subclass that never
   * overrides this produces permanent particles, cleaned up only by an
   * external `destroy()` / `destroyChildren()`. Override to add a speed
   * threshold, a life countdown, or any custom rule.
   */
