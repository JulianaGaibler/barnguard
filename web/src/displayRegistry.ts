...
/**
 * The lookup used by `main.ts` to resolve `?display=<id>` to a manifest. Each
 * entry is a dynamic import so unused displays never enter the bundle for a
 * given session (though on a kiosk with all displays available, they all end up
...
export const displayRegistry: Record<string, () => Promise<DisplayManifest>> = {
  ...
  arcade: () => import('./displays/arcade').then((m) => m.arcade),
}
